...
########################################################################################
########################################################################################
# This is now the second half of the script that's responsible for uploading the data. 
########################################################################################
########################################################################################


    def files(self): #write all files to be used for bulk inserts so functions can access and append to them as needed.
        print('writing tsv files for direct upload')
        ...
        with open('flank_records.tsv', 'w', newline='') as tsvfile:
            ...
            writer = csv.DictWriter(tsvfile, fieldnames=fieldnames)
            writer.writeheader()
        with open('PendingData.tsv', 'w', newline='') as tsvfile:
            writer = csv.writer(tsvfile, delimiter='\t', lineterminator='\n')
            fieldnames = ['accession_id','chrom_id', 'gene_id','flank_id', 'flank_strand','up_start', 'up_stop', 'down_start', 'down_stop'] #not including a strand column rn
            writer = csv.DictWriter(tsvfile, fieldnames=fieldnames)
            writer.writeheader()

    def PendingDataFiles(self): #make the name with a timestamp so they dont get overwritten if the person does multiple single uploads without checking them first?
        print('writing tsv files for reviews')
        dt = str(datetime.datetime.now())
        #change directory to sql uploads/pendingdata for review.
        os.chdir('/Users/biollab-120475/mysqlUploads/PendingData')  
        with open(f'P_gene_records_{dt}.tsv', 'w', newline='') as tsvfile: 
            writer = csv.writer(tsvfile, delimiter='\t', lineterminator='\n') #tab separated files are easily uploaded to MySQL
            fieldnames = ['sp_path', 'line_number', 'line_content']
            writer = csv.DictWriter(tsvfile, fieldnames=fieldnames)
            writer.writeheader()
        with open(f'P_exon_records_{dt}.tsv', 'w', newline='') as tsvfile:
            writer = csv.writer(tsvfile, delimiter='\t', lineterminator='\n')
            fieldnames = ['sp_path', 'line_number', 'line_content']
            writer = csv.DictWriter(tsvfile, fieldnames=fieldnames)
            writer.writeheader()
        with open(f'P_cds_records_{dt}.tsv', 'w', newline='') as tsvfile:
            writer = csv.writer(tsvfile, delimiter='\t', lineterminator='\n')
            fieldnames = ['sp_path', 'line_number', 'line_content']
            writer = csv.DictWriter(tsvfile, fieldnames=fieldnames)
            writer.writeheader()
    ...
    def csv_data(self): #Will run the loop for every genome. 
        ...
        print('queue:', self.successfully_downloaded_data)
    ...
    def exons(self):
        ...
        results = []
        self.exon_ss = []  # Reset with each chromosome
        grandparents = {}  # Dictionary for RNA-gene mapping

        # Preload RNA parents for the chromosome 
        ...
        for rna in rnas:
            try:
                ...
                p2 = p[-1].split(';')
                if len(p2) > 1:
                    rna_id = p2[0][3:]  # Extract RNA ID
                    if 'Parent=gene' in p2[1]:
                        gene_id = p2[1][7:]  # Extract gene ID
                        grandparents[rna_id] = gene_id
                    else:
                        print(f"\nMalformed RNA entry skipped: {rna}\n")
            except Exception as exc:
                print(f"\nError processing RNA grandparent: {rna} | {exc}\n")

        # Process exons
        exons = subprocess.run([f'grep -i -n "^{self.c_id}.*ID=exon.*" genomic.gff | less'],shell=True, stdout=subprocess.PIPE).stdout.decode('utf-8').rstrip().split('\n')
        for exon in exons:
            try:
                e0 = exon.split(':')  # Extract line number
                e = e0[1].split('\t')  # Extract fields
                e2 = e[-1].split(';')  # Extract attributes

                # Determine the parent gene ID
                gene_id = None
                if any('gene=' in attr for attr in e2):
                    gene_id = [attr.split('=')[1] for attr in e2 if 'gene=' in attr][0]
                elif any('Parent=gene' in attr for attr in e2):
                    gene_id = f"gene{[attr.split('=')[1] for attr in e2 if 'Parent=gene' in attr][0]}"
                elif any('Parent=id' in attr for attr in e2): #id is usually gene id if the previous 2 conditions arent there
                    gene_id = f"gene{[attr.split('=')[1] for attr in e2 if 'Parent=id' in attr][0]}"
                elif any('Parent=rna' in attr for attr in e2):
                    rna_id = [attr.split('=')[1] for attr in e2 if 'Parent=rna' in attr][0]
                    gene_id = grandparents.get(rna_id, None)

                # If a gene ID is determined, add the exon record
                if gene_id:
                    exon_rec = {'accession_id': self.sp_id,'chrom_id': self.c_id,'gene_id': gene_id,'exon_id': e2[0][3:],'exon_strand': e[6],'exon_seq_start': e[3],'exon_seq_end': e[4]}
                    results.append(exon_rec)
                else:
                    print(f"\nUnrecognized parent format: {e2}\n")

            except Exception as exc:
                print(f"\n\nError processing exon- record not added to file: {exon} | {exc}\n")
                chrom_update = """
                UPDATE Chromosomes
                SET PendingData = %s
                WHERE chrom_id = %s
                ...
                """
                with conn.cursor() as cursor:
                    cursor.execute(chrom_update, ('Y', self.c_id))
                    conn.commit()
                with conn.cursor() as cursor:
                    cursor.execute(sp_update, ('Y', self.sp_id))
                    conn.commit()
                #write record to file for review and manual upload   
                e0 = exon.split(':')
                P_ex_rec = {'sp_path': self.sp_path, 'line_number': e0[0], 'line_content': e0[1]} 
                os.chdir('/Users/biollab-120475/mysqlUploads/PendingData')
                with open('P_exon_records.tsv', 'a') as tsvfile: 
                    writer = csv.writer(tsvfile, delimiter='\t', lineterminator='\n')
                    writer.writerow(P_ex_rec.values())
        ...
        self.fix_overlap(self.exon_ss)
        self.fix_overlap(self.gene_ss)
    ...
    def cds(self, grandparents): #just like the exon function
        ...
        cdss = subprocess.run([f'grep -i -n "^{self.c_id}.*ID=cds.*" genomic.gff | less'], shell=True, stdout=subprocess.PIPE).stdout.decode('utf-8').rstrip().split('\n') #gives me all exons for chromosome
        for i in cdss: #for each cds
            try:
                c0 = i.split(':') #separate the line number
                c = c0[1].split('\t') #break it
                c2 = c[-1].split(';') #break the last field
            
                gene_id = None
                if any('gene=' in attr for attr in c2):
                    gene_id = [attr.split('=')[1] for attr in c2 if 'gene=' in attr][0]
                elif any('Parent=gene' in attr for attr in c2):
                    gene_id = f"gene{[attr.split('=')[1] for attr in c2 if 'Parent=gene' in attr][0]}"
                elif any('Parent=id' in attr for attr in c2): #id is usually gene id if the previous 2 conditions arent there
                    gene_id = f"gene{[attr.split('=')[1] for attr in c2 if 'Parent=id' in attr][0]}"
                elif any('Parent=rna' in attr for attr in c2):
                    rna_id = [attr.split('=')[1] for attr in c2 if 'Parent=rna' in attr][0]
                    gene_id = grandparents.get(rna_id, None)

                if gene_id:
                    cds_rec = {'accession_id': self.sp_id,'chrom_id': self.c_id,'gene_id': gene_id,'exon_id': c2[0][3:],'exon_strand': c[6],'exon_seq_start': c[3],'exon_seq_end': c[4]}
                    results.append(cds_rec)
                else:
                    print(f"\nUnrecognized parent format: {c2}\n")

            except Exception as exc:
                print(f"\n\nError processing exon- record not added to file: {i} | {exc}\n")
                chrom_update = """
                UPDATE Chromosomes
                SET PendingData = %s
                WHERE chrom_id = %s
                ...
                #write record to file for review and manual upload   
                c0 = i.split(':')
                P_cds_rec = {'sp_path': self.sp_path, 'line_number': c0[0], 'line_content': c0[1]} 
                os.chdir('/Users/biollab-120475/mysqlUploads/PendingData')
                with open('P_cds_records.tsv', 'a') as tsvfile: 
                    writer = csv.writer(tsvfile, delimiter='\t', lineterminator='\n')
                    writer.writerow(P_cds_rec.values())
        ...
        if len(self.exon_ss) > 1: #if there is more than one exon, so at least one seq
            for index, i in enumerate(self.exon_ss):
                if index < len(self.exon_ss)-1: #if its not the last exon
                    ...
                    key = self.intron_gids([start,stop]) #find the corresponding geneid and strand. i think this is slowing the function down... could i pass gid down through exon somehow?
...
today = numos() # leave this uncommented for single checks 

#today.check_records()
#today.do_mac_setup()
#today.check_for_new_data() 

today.files() # leave this uncommented for single checks 
today.PendingDataFiles() # leave this uncommented for single checks
#today.csv_data()
today.species('GCF_028640845.1', '/Users/biollab-120475/Desktop/ncbi_dataset/data/GCF_037176765.1/ncbi_dataset/data/GCF_037176765.1') # leave this uncommented for single checks, leave it out for running the whole program
#'GCF_028640845.1'  'GCF_037176765.1'
######GENERAL NOTES#######
# if you have to stop mysql to edit the configuration file, stop mysql first using system pref or the mysql command 'SHUTDOWN;'. Edit it here: /usr/local/mysql/etc/my.cnf   but when you restart sql, make sure the config file is set in the system pref to this: usr/local/mysql-9.0.1-macos14-x86_64/etc/my.cnf

############
#WORK TO DO#
############
#parent = id isnt adding to the database even though im trying to make it skip over that in the cds and exon functions. need to fix. should jump straight to 'gene='
# chromid NW_027118836.1 runs into a list index out of range error for no reason- print out some things to figure out whats up
#intron function takes a long time
#worried that gene ids might be the same at some point like, duplicate entries but on different chroms or even genomes. Make a unique combo restriction (chromid +geneid)for the database rather than unique across all geneids. 
#incorporate record writing and error catching in second half like you did in the first half. 
