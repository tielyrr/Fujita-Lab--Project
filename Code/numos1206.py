...
########################################################################################
    ...
    def exons(self):
        ...
        for i in exs: #for each exon
            ...
            if 'Parent=gene' in e2[1]:
                exon_rec = {'accession_id':self.sp_id, 'chrom_id':self.c_id, 'gene_id': e2[1][7::],'cds_id':e2[0][3::], 'cds_strand':e[6],'cds_seq_start':e[3], 'cds_seq_end':e[4]} #cant use self.gene_id because this is called outside the gene loop      
                ...
                for i in e2:
                    ...
                    if tf == False:
                        ...
                        tf_list.append('True')
                if 'True' in tf_list:
                    exon_rec = {'accession_id':self.sp_id, 'chrom_id':self.c_id, 'gene_id': f"gene-{i[6::]}",'exon_id':e2[0][3::], 'exon_strand':e[6],'exon_seq_start':e[3], 'exon_seq_end':e[4]} #search grandparents dict for rnaid to get gene parent
                    results.append(exon_rec)
                else:
                    if 'Parent=rna' in e2[1]:
                        exon_rec = {'accession_id':self.sp_id, 'chrom_id':self.c_id, 'gene_id': grandparents[e2[1][7::]],'cds_id':e2[0][3::], 'cds_strand':e[6],'cds_seq_start':e[3], 'cds_seq_end':e[4]}
                        results.append(exon_rec)
                    else:
                        print('new parent found, record not added to file:', e2) 
    ...
    def cds(self, grandparents): #just like the exon function
        ...
        for i in cdss: #for each cds
            ...
            if 'True' in tf_list:
                cds_rec = {'accession_id':self.sp_id, 'chrom_id':self.c_id, 'gene_id': f"gene-{i[6::]}",'exon_id':c2[0][3::], 'exon_strand':c[6],'exon_seq_start':c[3], 'exon_seq_end':c[4]} #search grandparents dict for rnaid to get gene parent
            ...
            if 'True' in tf_2list:
                ...
                if 'Parent=rna' in c2[1]:
                    ...
                    print('new parent found, record not added to file:', c2) 
