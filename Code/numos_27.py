...
class NCBI_genome_auto_update():
    def __init__(self):
        self.local_genomes_folder_pathway = r'/home/tielyr/database1/ncbi_dataset/data'
        self.local_genomes_jsonl_pathway = r'/home/tielyr/database1/ncbi_dataset/squamata.jsonl'  
        self.local_folder = r'/home/tielyr/database1/ncbi_dataset/' 
        self.taxon = 'squamata'
    ...
    def check_for_new_data(self): 
        ...
        os.chdir(self.local_folder) #make sure downloads is where we want it
 ...
 ###############################################################     
  ...
  # equality check, though, just in case they did not change the accession with the update. 
    ...
    def fix_dupes(self, update_lst): #currently only deletes duplicates from local files and not the database, also, the lab mac won't let me delete directories
        r_data = f'\nUpdates found for: {update_lst} --- Deleting these files(fna,lnk, gff) from {self.local_genomes_folder_pathway}.' #insert a function call here later onto perform the delete cascade simultaneously
        self.records_writing(r_data)
    ...
    def download_new_data(self, update_lst): #THIS TAKES TIME SINCE EACH ONE IS A HALF A GIG. Downloads one at a time since this is meant to be an updating script to an existing database. for uploads of bulk data, i may make a different script/function. 
        for i in self.data:
            ...
            if exit_value != 0: #if the download was unsuccessful or incomplete in any way
                ...
                self.successfully_downloaded_data.append(i['accession']) #necessary for child class to operate
                self.records_writing(r_data)
                    ...
                    for (root, dirs, files) in os.walk(self.local_genomes_folder_pathway): #this takes a long time even for just a few files...
                        ...
                        for name in dirs: #the gff files are in the directory and not individually named ('genomic.gff' with no accession included in name), this will add these along with any files in unzipped folders with the matching name
                            if i['accession'] in name:
                                ...
                                for j in stuff:
                                    if '.gff' in j:
                                        gff_path = os.path.abspath(os.path.join(root,j))
...
#these are commented out because I have been doing test uploads one by one with the classes below, not needing this one yet. 
#turn them back on to perform the check
#i havent tested these classes running one after another yet, though            
...
##############################################################################################################################################################################################
# while the parent class is inactive, a list called 'self.data' will take its place for testing purposes or when you dont need to use the update class                                       #   
#                                                                                                                                                                                            #
...
class adams_gff_gen(NCBI_genome_auto_update): #same functions you wrote, just in oop format
    def __init__(self):
        self.self = self    
...
class parse_upload(NCBI_genome_auto_update): #will write our new data to sql!
    def __init__(self):
        self.self = self
        self.data = ['GCA_009733165.1'] #change to be the parent class property successfully_downloaded_data when you need
        self.csv_path = r"C:\Users\tyler\Downloads\squamata_metadata.csv" #where the csv with the file paths is
        self.accession = '' #has to be above function calls. order of operations.
        self.gp = '' #gff path
        self.fnap = '' #fna path
        self.path_gen = self.gen_paths() #this doesnt work for multiple genomes, will need to change it to work as I intended
        self.gff_gen = self.gen_gff() #gff generator to iterate through, can't call 'next' if not saved as a variable
    ...
    def gen_paths(self): #spit out the path for the next accession(species) to be added from the csv-- have not tested if this works yet-- DOES NOT WORK- YOU SHOULD CALL THIS FOR EACH INDIVIDUAL THING
        df = pd.read_csv(self.csv_path)
        for i in self.data:
            d = {i: []} #make a dictionary with accession and list for paths
            index_of_accession = np.where(df["accession"] == i)
            g_p = f'{df.loc[index_of_accession]["path_to_gff"].item()}' #add the gff path to index 1 
            f_p = f"{df.loc[index_of_accession]['path_to_fna'].item()}" #add the fna path to index 2
            d[i].append(fr'{g_p}')
            d[i].append(f_p)
            self.accession = i #this is the accession we are on. 
            yield d[i] #each time this is called, it will change the accession and paths to the next one. make sure it is raw so it is read properly
            
    def gen_gff(self): #first thing called in the parse_write function. calls the path generator to set up the gff to be iterated over
        gg = adams_gff_gen()
        x = next(self.path_gen) #give me the next accession set. has to be saved as a variable in property before it can be iterated over
        self.gp = x[0] #assign paths to class properties
        self.fnap = x[1]
        ...
        return gg.lines(self.gp) #save gff generator of the file we are on to a property so it can be accessed by everything.
        ...
        for index, i in enumerate(self.gene_ss):
            if index != len(self.gene_ss)-1: #if it isnt the last gene
                if i[1] > self.gene_ss[index+1][0]: #if the second coordinate is bigger than the first of the next one, there is an overlapping gene
                    ...
                    r.append(list([i for i in range(self.gene_ss[y-1][0], int(self.gene_ss[y-1][1]+1))]))      #***********change to how i did this in the kb500?
    ...
    def parse_write(self):
        for line in self.gff_gen: #for each line in the gff generator
