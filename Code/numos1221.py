...
########################################################################################
    ...
    def exons(self):
        ...
        os.chdir(self.sp_path) #get to the directory with ur file
        exs = subprocess.run([f'grep -i -n "^{self.c_id}.*ID=exon.*" genomic.gff | less'], shell=True, stdout=subprocess.PIPE).stdout.decode('utf-8').rstrip().split('\n') #gives me all exons for chromosome
        for i in exs: #for each exon
            e0 = i.split(':')
            e = e0[1].split('\t') #break it
            e2 = e[-1].split(';') #break the last field
            for i in e2:
                tf_list = []
                tf_2list = []
                tf = 'gene=' in i #make this a list comprehension
                if tf == False:
                    tf_list.append('False')
                elif tf == True:
                    tf_list.append('True')
                tf2 = 'Parent=gene' in i
                if tf2 == False:
                    tf_2list.append('False')
                elif tf2 == True:
                    tf_2list.append('True')
            if 'True' in tf_list:
                idx = tf_list.index('True')
                exon_rec = {'accession_id':self.sp_id, 'chrom_id':self.c_id, 'gene_id': f"gene-{idx[6::]}",'exon_id':e2[0][3::], 'exon_strand':e[6],'exon_seq_start':e[3], 'exon_seq_end':e[4]} #cant use self.gene_id because this is called outside the gene loop      
                results.append(exon_rec)
            if 'True' in tf_2list:
                idx = tf_2list.index('True')
                exon_rec = {'accession_id':self.sp_id, 'chrom_id':self.c_id, 'gene_id': e2[idx][7::],'exon_id':e2[0][3::], 'exon_strand':e[6],'exon_seq_start':e[3], 'exon_seq_end':e[4]} #cant use self.gene_id because this is called outside the gene loop      
                results.append(exon_rec)
            else:
                if 'Parent=rna' in e2[1]:
                    ...
                    results.append(exon_rec)
                elif 'Parent=id' in e2[1]: #if theres no direct gene reference, the geneid is the parent id and usually also in the exon id
                    exon_rec = {'accession_id':self.sp_id, 'chrom_id':self.c_id, 'gene_id': f'gene{e2[1][11::]}', 'exon_id':e2[0][3::], 'exon_strand':e[6],'exon_seq_start':e[3], 'exon_seq_end':e[4]}
                    results.append(exon_rec)
                else:
                    print('new parent found, record not added to file:', e0) 
                    chrom_update = """
                    UPDATE Chromosomes
                    SET PendingData = %s
                    WHERE chrom_id = %s
            ...
            if len(c0) > 1:
                ...
                for i in c2:
                    ...
                    if tf2 == False:
                        ...
                        tf_2list.append('True')
                if 'True' in tf_list: #find the index where 'true' is and thatll  be the index for the record??
                    idx = tf_list.index('True')
                    cds_rec = {'accession_id':self.sp_id, 'chrom_id':self.c_id, 'gene_id': f"gene-{idx[6::]}",'cds_id':c2[0][3::], 'cds_strand':c[6],'cds_seq_start':c[3], 'cds_seq_end':c[4]} #search grandparents dict for rnaid to get gene parent
                    results.append(cds_rec)
                if 'True' in tf_2list:
                    idx = tf_list.index('True')
                    cds_rec = {'accession_id':self.sp_id, 'chrom_id':self.c_id, 'gene_id': c2[idx][7::],'cds_id':c2[0][3::], 'cds_strand':c[6],'cds_seq_start':c[3], 'cds_seq_end':c[4]} #cant use self.gene_id because this is called outside the gene loop      
                    results.append(cds_rec)
                else:
                    if 'Parent=rna' in c2[1]:
                        ...
                        results.append(cds_rec)
                    elif 'Parent=id' in c2[1]: #if theres no direct gene reference, the geneid is the parent id and usually also in the exon id
                        cds_rec = {'accession_id':self.sp_id, 'chrom_id':self.c_id, 'gene_id': f'gene{c2[1][11::]}', 'cds_id':c2[0][3::], 'cds_strand':c[6],'cds_seq_start':c[3], 'cds_seq_end':c[4]}
                        results.append(cds_rec)
                    else:
                        print('new parent found, record moved to review file:', c2) 
                        chrom_update = """
                        UPDATE Chromosomes
                        SET PendingData = %s
                        WHERE chrom_id = %s
...
today.PendingDataFiles()
today.csv_data()
#today.species('GCF_037176765.1', '/Users/biollab-120475/Desktop/ncbi_dataset/data/GCF_037176765.1/ncbi_dataset/data/GCF_037176765.1') # leave this uncommented for single checks, leave it out for running the whole program
...
def cds(self, grandparents): #just like the exon function
        ...
        for i in cdss: #for each cds
            ...
            if len(c2) > 1:
                if 'gene=' in c2[5]:
                    cds_rec = {'accession_id':self.sp_id, 'chrom_id':self.c_id, 'gene_id': f"gene-{c2[5][4::]}",'exon_id':c2[0][3::], 'exon_strand':c[6],'exon_seq_start':c[3], 'exon_seq_end':c[4]} #search grandparents dict for rnaid to get gene parent
                    #print(cds_rec)
                    results.append(cds_rec)
                    ...
                    print('new parent found, record not added to file:', i) #theres one where gene= is on c4 not c5
            else:
