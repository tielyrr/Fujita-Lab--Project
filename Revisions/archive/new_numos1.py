...
########################################################################################
    ...
    def exons(self):
        ...
        for i in exs: #for each exon
            ...
            if len(e2) >1:
                if 'gene='in e2[5]:
                    exon_rec = {'accession_id':self.sp_id, 'chrom_id':self.c_id, 'gene_id': f"gene-{e2[5][4::]}",'exon_id':e2[0][3::], 'exon_strand':e[6],'exon_seq_start':e[3], 'exon_seq_end':e[4]} #search grandparents dict for rnaid to get gene parent
                    #print(exon_rec)
                    ...
                    print(f'new parent found. record {i} not added to database') #theres one where gene= is on c4 not c5. find a way to search the content of the keys quickly rather than these if statements.
            else:
...
#WORK TO DO#
############
#uploading cds for NC_090026.1 ran into error and sql might have deleted table data because exon count went down to 180...: pymysql.err.OperationalError: (1206, 'The total number of locks exceeds the lock table size')
#change exon and cds criteria to search content of value and not the whole value to avoid tons of if statements
